...
## Giai đoạn 3 — Phân quyền (Authorization / IDOR) ✅ Đã xong (review xong; F-11 chờ vá ở Giai đoạn 12)
...
- **F-14 [High]** `xlsx@0.18.5` (bản cuối trên npm registry, SheetJS không vá trên npm nữa) dính CVE-2023-30533 (prototype pollution khi parse file Excel độc) và CVE-2024-22363 (ReDoS); app parse file user upload tại 3 endpoint import BOQ/serial (sau đăng nhập, giới hạn 5–10MB — giảm nhẹ, không loại trừ). **Hướng vá (9.2):** cài bản ≥ 0.20.2 từ cdn.sheetjs.com (`npm i https://cdn.sheetjs.com/xlsx-0.20.2/xlsx-0.20.2.tgz`) hoặc chuyển `exceljs`; nhớ đổi cả import phía frontend (warranty import parse client-side, cùng package). — **chưa vá (xử lý ở 9.2)**
