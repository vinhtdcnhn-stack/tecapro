...
- **F-01 [Critical]** `TELEGRAM_BOT_TOKEN` lộ trong git history của repo **public** (`.env` tại commit `cfa456b` và các commit sau, đã xoá khỏi working tree ở `4685f69` nhưng history vẫn giữ). Ai cũng có thể lấy token và điều khiển bot (gửi tin nhắn giả mạo tới user, đọc update của bot). **Khắc phục:** ① Revoke token NGAY qua @BotFather (`/revoke`) và cấp token mới chỉ đặt trong `.env` local + VPS; ② cân nhắc chuyển repo sang private; ③ nếu muốn sạch hẳn history: dùng `git filter-repo` xoá `.env` khỏi mọi commit rồi force-push (sau khi đã revoke thì bước này là tuỳ chọn). — **chưa vá** (cần bạn thao tác với BotFather)
- **F-02 [Medium]** `AUTH_SECRET` hiện 32 ký tự dạng human-pattern (kèm dấu backtick bao quanh trong file), yếu hơn khuyến nghị 48-byte random của chính codebase. Chưa từng lộ ra git. **Khắc phục:** tạo lại bằng `node -e "console.log(require('crypto').randomBytes(48).toString('hex'))"`, đặt giá trị KHÁC NHAU cho local và VPS. Lưu ý: đổi secret sẽ logout toàn bộ phiên hiện có. — **chưa vá**
