...
//
// Trả về { ct, dw, tn, inbox } và tự bật/tắt class nền đỏ cảnh báo trên <body>:
...
//   • tender-page-alert khi có mục đầu việc đấu thầu chưa đọc (server tự lọc theo liên quan)
//
// Các nơi sau khi GHI vẫn phát event để ép poll lại NGAY (không chờ server đẩy):
// 'contracttask:refresh-unread', 'deptwork:refresh-unread', 'tender:refresh-unread',
// 'approval:refresh-inbox', focus.
const ZERO = { ct: 0, dw: 0, tn: 0, inbox: 0 }
...
export default function useLiveAlerts(user) {
  ...
  useEffect(() => {
    ...
    const poll = async (known) => {
      ...
      try {
        const r = await fetch(
          `${API}/live/poll?ct=${known.ct}&dw=${known.dw}&tn=${known.tn}&inbox=${known.inbox}`,
        ...
        if (!alive) return
        const next = { ct: Number(d.ct) || 0, dw: Number(d.dw) || 0, tn: Number(d.tn) || 0, inbox: Number(d.inbox) || 0 }
        setCounts(next)
    ...
    window.addEventListener('tender:refresh-unread', forceRefresh)
    ...
    return () => {
      ...
      window.removeEventListener('tender:refresh-unread', forceRefresh)
  ...
  // Việc đấu thầu chưa đọc: server đã tự lọc theo người liên quan (tn = 0 với người
  // không liên quan) nên không cần cổng thêm ở client.
  useEffect(() => {
    document.body.classList.toggle('tender-page-alert', active && view.tn > 0)
    return () => document.body.classList.remove('tender-page-alert')
  }, [active, view.tn])

  // Tổng số việc CHƯA ĐỌC dùng cho cảnh báo logo (CSS, theo class body ở trên) và nhấp
  // nháy tiêu đề tab: việc HĐ (mọi user) + việc phòng (admin / Ban KT Cơ điện) + đấu thầu.
  const unread = (active ? view.ct : 0) + (dwAlertActive ? view.dw : 0) + (active ? view.tn : 0)
