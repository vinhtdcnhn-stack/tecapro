...
import { PRIORITIES, STATUSES, statusModalClass } from './taskConstants'
...
export default function TaskModal({ task, departments, users, currentUser, onSave, onClose }) {
  ...
  const AttachIcon = () => (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="attach-icon">
          ...
          <div className="task-form-row">
            <div className="task-form-group full">
              ...
              <input type="text" value={form.title} onChange={e => set('title', e.target.value)}
                placeholder="Nhập tên công việc..." className={errors.title ? 'has-error' : ''} />
            ...
            <div className="task-form-group">
              ...
              {form.department_id && deptUsers.length === 0 && <span className="assign-hint">Phòng ban này chưa có nhân viên</span>}
              {!form.department_id && <span className="assign-hint">Chọn phòng ban để lọc nhân viên</span>}
              ...
              <div className="status-radio-group">
                {STATUSES.map(s => (
                  <div key={s} className={`status-radio-opt ${form.status === s ? statusModalClass(s) : ''}`}
                    onClick={() => set('status', s)}>{s}</div>
                ))}
              </div>
              <label>Ghi chú</label>
                ...
                {attachments.map(att => (
                  <div key={att.id} className="task-attach-item task-attach-item--saved">
                    ...
                    <a href={att.file_path}
                      ...
                      className="attach-name" title={att.file_name}>
                      {att.file_name}
                    </a>
